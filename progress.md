...
- **#1** Parts of the 2026-05 expansion (commit list in `docs/updates` U-20260922-05) are still covered only by `test/test_expansion_smoke.py` and `test/test_expansion2_smoke.py` (workspace S-2): the probes in `je_load_density/utils/security/` (fuzz, graphql, jwt, owasp, rate-limit, smuggling, ssrf), and `editors/` (VS Code, Chrome, JetBrains), whose builds no check runs. The JetBrains plugin builds locally with Gradle 8.10.2 and has a lockfile (U-20260923-26), but has no Gradle wrapper and no CI job. `deploy/` is checked by `deploy-check.yml` (U-20260923-25).
